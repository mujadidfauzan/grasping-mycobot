...
DEFAULT_XML_PATH = PROJECT_ROOT / "source" / "robot" / "robot.xml"
...
class MyCobotIK:
    def __init__(
        ...
        if self.ee_site_id < 0:
            raise ValueError(f"Site end-effector `{self.ee_site_name}` tidak ditemukan.")
    ...
    def home_configuration(self) -> np.ndarray:
        home = np.zeros(len(self.joint_names), dtype=np.float64)
    ...
    def has_site(self, site_name: str) -> bool:
        site_id = int(mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_SITE, site_name))
        return site_id >= 0
    ...
    def site_pose(self, site_name: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        return self.data.site(site_name).xpos.copy(), _normalize_quat(quat)

    def end_effector_pose(self, q_rad: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray]:
    ...
    def solve(
        ...
        *,
        ...
        if use_rotation:
            target_quat = _normalize_quat(np.asarray(target_quat, dtype=np.float64).reshape(4))
        else:
            target_quat = None

        if initial_q is None:
            initial_q = self.home_configuration()
        initial_q = self._clip_to_limits(np.asarray(initial_q, dtype=np.float64).reshape(-1))
        if initial_q.shape != (len(self.joint_names),):
            raise ValueError(
                f"initial_q harus berisi {len(self.joint_names)} joint, "
                f"namun menerima shape {initial_q.shape}."
            )
        ...
        for attempt, seed_q in enumerate(seeds):
            ...
            for iteration in range(1, int(max_iters) + 1):
                ...
                damp_matrix = (float(damping) ** 2) * np.eye(jacobian.shape[0], dtype=np.float64)
                try:
                ...
                q_rad = self._clip_to_limits(q_rad + float(step_size) * delta_q)
            ...
            score = float(
                ...
                + (0.0 if target_quat is None else rotation_weight * np.linalg.norm(rot_error))
            )
            if score < best_score:
                best_score = score
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path ke XML MuJoCo. Default: source/robot/robot.xml",
    )
    parser.add_argument(
        "--ee-site-name",
...
def _resolve_initial_joints(args: argparse.Namespace) -> np.ndarray | None:
    if args.initial_joints_rad is not None and args.initial_joints_deg is not None:
        raise ValueError("Pilih salah satu: --initial-joints-rad atau --initial-joints-deg.")
...
def render_solution(
    ...
    *,
    ...
    except ModuleNotFoundError as exc:  # pragma: no cover - viewer depends on runtime env
        raise ModuleNotFoundError(
